...
Add `BOT_TOKEN_1` through `BOT_TOKEN_5` as secret environment variables in Render. The web service listens on Render's `PORT` value.

To join voice channels on startup, add `GUILD_ID_1` and `VOICE_CHANNEL_ID_1` for bot 1, through `GUILD_ID_5` and `VOICE_CHANNEL_ID_5` for bot 5. Each bot may use a different server and voice channel. The dashboard also has a voice connection section for changing these destinations at runtime. Enable Developer Mode in Discord to copy server and channel IDs, and grant each bot View Channel, Connect, and Speak permissions.
